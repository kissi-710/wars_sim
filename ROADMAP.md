...
Phase 0〜9(UI 刷新、「決着がつかない」問題の解決まで)は完了(残りは末尾の「残タスク」)。`npm run typecheck` / `npm test`(69 件)/ `npm run build` はすべて通る。
...
npx tsx scripts/stalecheck.ts     # 決着/収束中/膠着の分類(例: npx tsx scripts/stalecheck.ts 100 4000)
```
...
## Phase 9 — 「決着がつかない」問題の解決

原因を調査した結果、3 パターンの膠着があった: ①建物を失った残党に攻撃目標が無い ②残党が建物に囲まれて物理的に孤立する ③互角の相手同士が永遠に様子見する。詳細は docs/spec.md §15.2〜15.3。

- [x] **残党狩り**(`fields.ts`): 攻撃目標の相手に建物が無ければ、残存領土 → ユニット位置の順に直接の攻撃目標にする
- [x] **閉じ込め防止**(`construction.ts` の `wouldSeal`): 建設で、隣接するどのマス(所有国問わず)も 4 方向すべて建物になる配置を避ける
- [x] **戦争疲れ**(`strategy.ts`): 国境を接した相手への開戦を見送るたびにしきい値が下がり(下限あり)、開戦するとリセット
- [x] **孤立崩壊**(新規 `systems/collapse.ts`): 建物 0 が 150 ターン続く国は、ユニットが毎ターン弱って消滅する(保険的な最終手段)
- [x] UI: 国カードに「孤立崩壊中」バッジ、国の詳細に建物 0 の経過ターン数を表示
- [x] テスト 14 件追加(`tests/stalemate.test.ts`)。65→69 件
- [x] `scripts/stalecheck.ts`(新規): 決着/収束中/膠着を分類する検証スクリプト
- [x] 検証: 3 国戦 100 シードで、決着率が 1,500 ターン時点 20〜25% → 49%、4,000 ターン時点で 92% に改善。性格バランス(2,000 ターン)も 20〜45% の目標内(好戦的 31% / 中庸 34% / 平和的 35%)

---

## 残タスク・課題(優先順)

1. 実機でのブラウザ確認(MAX 再生、タッチ操作、`devicePixelRatio` 2 のにじみ)。
2. 4,000 ターンでも約 5〜8% は決着しない(§15.3)。ほとんどは防衛側の最後の拠点をゆっくり削っているだけの進行中の消耗戦で、真の膠着ではないと見られるが、さらに長い期間での再検証はできていない。
3. 国名・国旗、色覚への配慮(頭文字/記号)、セーブ/ロード・リプレイ。
